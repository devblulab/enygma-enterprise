...
const ListPost: React.FC<{ setItems: React.Dispatch<React.SetStateAction<Item[]>> }> = ({ setItems }) => {
  ...
  const handleAddItem = async () => {
    try {
      ...
      const itemSalvo = await colecao.salvar('BetodespachanteTransferenciaArquivos', itemParaSalvar);
...
<Grid item xs={12} md={3}>
  ...
  onChange={(e) => {
    ...
    // Atualiza o id do documento com base no CNPJ (opcional: pode ser hash, timestamp, etc)
    handleInputChange({
      ...e,
      target: {
        ...e.target,
...
/>
          ...
          <Grid item xs={12} md={3}>
            <Typography variant="h6" className={classes.sectionTitle}>Identificação Comprado</Typography>
            <TextField
              name="cpfvendedor"
              label="CPF"
